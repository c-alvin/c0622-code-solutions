...
export default class App extends React.Component {
  ...
  componentDidMount() {
     ...
     */
    fetch('/api/todos').then(res => res.json()).then(data => this.setState({
      todos: data
    }));
  }
  ...
  toggleCompleted(todoId) {
    ...
    for (let i = 0; i < this.state.todos.length; i++) {
      if (todoId === this.state.todos[i].todoId) {
        obj.opposite = !this.state.todos[i].isCompleted;
        index = i;
      }
    }
    fetch(`/api/todos/${todoId}`, { method: 'PATCH', body: JSON.stringify({ isCompleted: obj.opposite }), headers: { 'Content-Type': 'application/json' } })
      .then(res => res.json())
      .then(data => {
        const copy = [...this.state.todos];
